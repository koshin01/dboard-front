...
export default function Board() {
    ...
    return (
        <>
            {approvalState && (
                <button disabled type="button" class="py-2.5 px-5 mr-2 text-sm h-20 text-gray-900 font-bold border border-gray-300 rounded-lg inline-flex items-center">
            ...
            {formattedMessages.slice(0).reverse().map((message, index) => {
                return (
                    <div key={index}>
                        <div className = "text-emerald-500">{message.address}</div>
                        <div className = "text-gray-500">{message.timestamp.toString()}</div>
                        <div className = "font-medium">{message.context}</div>
                    </div>
                );
            })}
        </>
